...
目标成片 12–14 分钟，完整展示“从模板建立项目规则，收到供应商交付，记录项目上下文，审计，定位证据，生成整理计划，拦截冲突，批准执行，复检并查看收据”。
...
进入“项目规则”，展示环境资产和角色资产两个版本化模板。点击“从模板新建”，说明命名正则、检查格式、贴图通道和保留版本数都可以由审核人员配置，不必手写 JSON。

把命名正则临时改成 `[`，展示红色字段错误和禁用的保存按钮；恢复正确正则。强调 Profile 只能保存到交付目录之外，应用新 Profile 后旧审计与旧整理计划会失效。
